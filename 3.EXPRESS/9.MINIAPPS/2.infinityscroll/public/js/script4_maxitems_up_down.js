...
let totalEndIndex = totalStartIndex + itemsPerLoad;
...
window.addEventListener("scroll", async () => {
  ...
  //단 위로 스크롤할때 h1으로 올라가기 직전에 판단해야한다.
  const h1Tag = document.getElementById('h1_title');
  const h1Height = h1Tag.getBoundingClientRect().height;
  const h1Margin = parseFloat(window.getComputedStyle(h1Tag).marginTop) + parseFloat(window.getComputedStyle(h1Tag).marginBottom);
  const h1TotalHeight = h1Height + h1Margin;
  ...
  if (
    ...
    totalEndIndex += data.length;

    isLoadingDown = false;
  }
  //스크롤이 현재화면 가장 위에 도달했으면
  else if (!isLoadingUp && window.scrollY <= h1TotalHeight) {
    console.log("화면 맨 위 이동");

    if (totalStartIndex == 0){
        return;
    }
    ...
    result.children[itemsPerLoad - 1].scrollIntoView({
      behavior: "auto",
      block: "start",
    });
  ...
  console.log(`totalStartIndex:${totalStartIndex}, totalEndIndex:${totalEndIndex}`);
});
