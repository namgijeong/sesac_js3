...
function myLogger(option) {
    ...
    //closure 클로저 => 함수에 인자를 쓰지 않아도 나의 스코프를 포함해서 판단한다
    //return function (req, res, next, option) { 
    return function (req, res, next) {
        let dateString = new Date().toLocaleDateString();
        //req.path 대신 req.originalUrl 도 가능

        let logString;
        if (option == 'dev') {
            logString = `DEV [${dateString}] [${req.method}] [${req.path}] `;
        } else {
            logString = `RELEASE [${dateString}] [${req.method}] [${req.path}] `;
        }
